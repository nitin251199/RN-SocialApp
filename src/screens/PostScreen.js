...
export default function PostScreen({route}) {
  const {header, content, images, actions} = route.params;
      ...
      <View style={styles.comment}>
        <View style={{flexDirection: 'row'}}>
          ...
          <View style={styles.commentContent}>
            ...
            <Text style={{fontSize: 15, marginBottom: 3, color: GRAY}}>
              {item.comment}
            </Text>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              ...
              <Text style={{fontSize: 15, color: LIGHT_GRAY}}>
        ...
        <Pressable onPress={() => setLiked(!isLiked)}>
          <ICONS.LIKE size={18} color={isLiked ? SOCIAL_BLUE : GRAY} />
        </Pressable>
      </View>
    );
  };
  ...
  const ListHeaderComponent = () => (
    <>
      <Header
        title={
          <Text style={{fontSize: 16, letterSpacing: 1.3, color: DARK_BLACK}}>
            POST
          </Text>
        }
      ...
      <View style={styles.commentSection}>
        <View
          ...
          <Text style={{color: DARK_BLACK, fontSize: 15, letterSpacing: 0.7}}>
            COMMENTS (45)
          </Text>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <Text
              style={{marginRight: 5, fontWeight: '600', color: DARK_BLACK}}>
              Recent
            </Text>
  ...
  const ListEmptyComponent = () => (
    <View
      ...
      <Text style={{fontSize: 20, color: DARK_BLACK}}>
